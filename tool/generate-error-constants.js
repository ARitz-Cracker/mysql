...
var srcDirMaria = process.argv[4];
if (!srcDir5 || !srcDir8 || !srcDirMaria) {
  var args = [];
  ...
  args[4] = path.join('path', 'to', 'mariadb', 'src');
...
var versionMaria = extractMySqlVersion(srcDirMaria);
...
appendGlobalErrorCodes(srcDirMaria, codes);
appendDatabseErrorCodes(srcDir5, codes);
appendDatabseErrorCodes(srcDir8, codes);
appendDatabseErrorCodes(srcDirMaria, codes);
...
appendSqlErrorCodes(srcDirMaria, codes, path.join(srcDirMaria, 'sql', 'share', 'errmsg-utf8.txt'));
...
function appendSqlErrorCodes(srcDir, codes, errorFile) {
  ...
  var sections  = contents.split(/^(start|skip-to)-error-number (\d+)$/m);
  for (var i = 2; i < sections.length; i += 3) {
    var offset = Number(sections[i]);

    // The empty array default accounts MariaDB's blank 2xxx error code definitions
